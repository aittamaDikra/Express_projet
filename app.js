...
app.post('/visite/',(req, res) => { 
    let data = {id_m: req.body.idd, longitude: req.body.long, etat: 1, latitude: req.body.latt};
...
app.get('/test/',(req, res) => {
    // res.send('CRUD Operation using NodeJS / ExpressJS / MySQL');
    let sql = "SELECT visite.id,nom,etat,date FROM visite,monuments where visite.id_m=monuments.id";
    let query = connection.query(sql, (err, rows) => {
        if(err) throw err;
